...
Read the pushed BUILD-18 code/report and its predecessor evidence. Pin the actual code commit and application tree; identify whether the reviewer is independent or the implementer. Do not infer a pass from CI or from this planning card.
...
7. Inspect recovery: Code completion and field acceptance are separate statuses; missing device/printer/participant evidence keeps the pilot release blocked.

## Cumulative milestone gate

Run J01–J07 and J11 on the named pilot phone/browser, with real printer output, native Hindi review, installed-PWA offline/update testing and an owner recovery drill. Missing hardware, participant or deployment evidence blocks the associated pilot claim.

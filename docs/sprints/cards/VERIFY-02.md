...
Read the pushed BUILD-02 code/report and its predecessor evidence. Pin the actual code commit and application tree; identify whether the reviewer is independent or the implementer. Do not infer a pass from CI or from this planning card.
...
4. Check **every prior build**: [BUILD-01](BUILD-01.md).
...
7. Inspect recovery: List every mutation caller and failure path; fall back to the tested adapter only if no newer-schema records were written.

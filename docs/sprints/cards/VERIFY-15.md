...
Read the pushed BUILD-15 code/report and its predecessor evidence. Pin the actual code commit and application tree; identify whether the reviewer is independent or the implementer. Do not infer a pass from CI or from this planning card.
...
7. Inspect recovery: Typed fixtures can ship first; actual recorded-audio and target-phone evidence is a separate required gate before an accuracy claim.

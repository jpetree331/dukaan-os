...
test('B16: birthday, briefing and CSV icon HTML are rendered as text',async()=>{
  ...
  let html;A.el=s=>{html=s;return node('summary');};A.modal=()=>{};A.morningBrief(true);
  assert.ok(!html.includes(marker));assert.match(html,/&lt;img/);
  csvImport(h,'name,price,emoji\nImported,100,"'+marker.replace(/"/g,'""')+'"');
...
test('B20: item questions preserve whole words and yesterday excludes today',async()=>{
  ...
  const yesterday=A.actions.checkout(cart(maggi));yesterday.at=Date.now()-A.DAY;
  A.actions.checkout(cart(oreo,{lines:[{itemId:oreo.id,qty:2,price:100}]}));
...
test('B29: blank CSV numeric fields preserve values; invalid rows abort all changes; Hindi and GST update',async()=>{
  const h=await create(),{A}=h;const i=item(A,{name:'Original',nameHi:'पुराना'});A.save();
  const before=JSON.stringify(A.DB());
  assert.throws(()=>csvImport(h,'name,nameHi,price,cost,stock,gst\nOriginal,नया,,bad,,18'),/cost/);
  assert.equal(JSON.stringify(A.DB()),before);
  csvImport(h,'name,nameHi,price,cost,stock,gst\nOriginal,नया,,,,18');
  ...
  csvImport(h,'name,price,cost,stock,gst\nOriginal,0,0,0,0');
...
test('Existing sample data remains valid through save and reload',async()=>{
  const {A}=await create();A.seed();const count=A.DB().bills.length;
  A.boot('audit');assert.ok(count>100);assert.equal(A.DB().bills.length,count);
});
...
test('Reported CSV/receipt limitations: formula text, complete history, void marker and paise',async()=>{
  ...
  for(let k=0;k<45;k++) A.actions.checkout(cart(i,{customerId:c.id}));
  const first=A.DB().bills[0];A.actions.voidBill(first.id);
...
test('Reported expired-stock limitation: quarantine quantities without losing physical stock',async()=>{
  ...
  A.save();assert.equal(A.itemStock(i),3);assert.equal(A.sellableStock(i),1);
  assert.throws(()=>A.actions.checkout(cart(i,{lines:[{itemId:i.id,price:100,qty:2}]})),/Insufficient/);
  const b=A.actions.checkout(cart(i));assert.equal(A.itemStock(i),2);assert.equal(A.sellableStock(i),0);
  assert.equal(b.lines[0].cost,30);assert.match(A.ai.expiry()[0].text,/cannot be sold/);
  A.actions.voidBill(b.id);assert.equal(A.sellableStock(i),1);assert.equal(A.itemStock(i),3);
});

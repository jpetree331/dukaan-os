...
test('B26–B27: server allowlist and malformed requests; process remains healthy',async()=>{
  ...
  try{
    ...
    const get=(p)=>new Promise((resolve,reject)=>http.get({host:'127.0.0.1',port,path:p},r=>{let body='';r.on('data',x=>body+=x);r.on('end',()=>resolve({status:r.statusCode,body,headers:r.headers}));}).on('error',reject));
    ...
    for(const [key,value] of Object.entries(require('../security-headers.cjs'))) assert.equal(home.headers[key.toLowerCase()],value);
    assert.equal(home.status,200);assert.equal((await get('/js/safety.js')).status,200);
...
test('B28: service worker preserves unrelated caches and includes the complete shell',async()=>{
  ...
  const prefix='dukaan-os-'+encodeURIComponent('https://example.test/')+'-';
  const ctx={URL,self:{registration:{scope:'https://example.test/'},addEventListener:(e,f)=>handlers[e]=f,skipWaiting:async()=>{},clients:{claim:async()=>{}},location:{origin:'https://example.test'}},
    caches:{keys:async()=>[prefix+'old',prefix+'security-v1','dukaan-os-other-scope-old','other-app'],delete:async k=>deleted.push(k),open:async()=>({addAll:async paths=>shell=paths})}};
  vm.runInNewContext(fs.readFileSync(path.join(ROOT,'sw.js'),'utf8'),ctx);
  handlers.activate({waitUntil(p){task=p;}});await task;assert.deepEqual(deleted,[prefix+'old']);

...
test('VERIFY-01: fractional stock survives CSV update and failed multi-line purchase',async()=>{
  ...
  (await A.importItemRows([{name:i.name,stock:1.0001}]));
  assert.equal(A.item(i.id).stock,1.0001);
  const before=JSON.stringify(A.DB());
  (await assert.rejects(async ()=>(await A.actions.recordPurchase(s.id,[{itemId:i.id,qty:1,cost:60},{itemId:i.id,qty:0.00011,cost:60}],0)),/four decimal/));
  assert.equal(JSON.stringify(A.DB()),before);
  (await assert.rejects(async ()=>(await A.importItemRows([{name:i.name,stock:2},{name:'bad',price:1,stock:0.00001}])),/four decimal/));
  assert.equal(JSON.stringify(A.DB()),before);
});
...
test('VERIFY-01: persisted snapshots, command links and duplicate void survive reload',async()=>{
  ...
  const b=(await A.actions.checkout(cart(i)));
  // Explicitly emulate an old bill lacking the new extension metadata.
  delete b.command;delete b.calculationVersion;(await A.save());
  ...
  (await B.actions.voidBill(b.id));assert.equal(B.DB().bills[0].voidCommand.corrects,b.id);
  const state=JSON.stringify(B.DB());
  assert.equal((await B.actions.voidBill(b.id)),null);assert.equal(JSON.stringify(B.DB()),state);
  const newer=(await B.actions.checkout(cart(B.item(i.id))));

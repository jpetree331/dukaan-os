...
test('B13: schema/reference/version/counter validation and quota-safe restore',async()=>{
  const {A,storage,ctx}=await create();item(A);(await A.save());
  ...
  for(const mutate of [d=>d.stores=[],d=>d.v=99,d=>d.items[0].price=-1,d=>d.items[0].id='" onclick="x',
    ...
    const d=JSON.parse(before);mutate(d);(await assert.rejects(async ()=>(await A.restoreBackup(d))));
  ...
  (await assert.rejects(async ()=>(await A.restoreBackup(next)),/quota/));assert.equal(A.items()[0].name,valid.items[0].name);
  assert.equal(storage.get('dukaanos.v2.audit'),before);
  ctx.localStorage.setItem=write;(await A.restoreBackup(next));assert.equal(A.items()[0].name,'Restored item');
...
test('B14: exclusive writer lock and stale-snapshot guard prevent lost bills',async()=>{
  ...
  const storage=new Map(),a=await create(storage,{locks});const i=item(a.A);(await a.A.save());
  const b=await create(storage,{locks});(await assert.rejects(async ()=>(await b.A.boot('audit')),/not writable/));
  (await a.A.actions.checkout(cart(i)));assert.equal(JSON.parse(storage.get('dukaanos.v2.audit')).bills.length,1);
  ...
  (await assert.rejects(async ()=>(await a.A.actions.checkout(cart(i))),/another tab/));
...
test('B15: checkout is durable before returning; failed write rolls back stock, bill, points and counter',async()=>{
  const {A,ctx,storage}=await create();const i=item(A),c=customer(A,{points:20});(await A.save());
  const before=storage.get('dukaanos.v2.audit'), write=ctx.localStorage.setItem;
  ctx.localStorage.setItem=()=>{throw Error('quota');};
  (await assert.rejects(async ()=>(await A.actions.checkout(cart(i,{customerId:c.id,redeem:20}))),/quota/));
  assert.equal(i.stock,10);assert.equal(c.points,20);assert.equal(A.DB().bills.length,0);assert.equal(A.DB().counter.bill,1);
  assert.equal(storage.get('dukaanos.v2.audit'),before);
  ctx.localStorage.setItem=write;const b=(await A.actions.checkout(cart(i)));
...
test('B17: legacy queue is preserved and never reports a fake upload',async()=>{
  ...
  ctx.navigator.onLine=true;(await A.save({op:'test'}));A.sync.drain();flush(700);
...
test('B23: login migration updates memory immediately; later save cannot erase migrated shop',async()=>{
  ...
  (await A.auth.enableGate(acc.id));assert.equal(A.items().length,1);(await A.save({sync:false}));
  assert.equal(JSON.parse(storage.get('dukaanos.v2.'+acc.id)).items.length,1);
  (await A.auth.disableGate());(await A.save());assert.equal(A.accountId,'local');assert.equal(JSON.parse(storage.get('dukaanos.v2.local')).items.length,1);

...
test('B01–B03: mixed stock, last batch depletion, and exact void allocations', async () => {
  ...
  A.actions.restock(i.id,5,'2027-01-01',70);
  assert.equal(A.itemStock(i),15);
  const before=JSON.stringify(i.batches.slice().sort((a,b)=>a.id.localeCompare(b.id)));
  const b=A.actions.checkout(cart(i,{lines:[{itemId:i.id,qty:15,price:100}]}));
...
test('B04, B21: foreign, deleted, stale price, duplicate and over-stock lines cannot charge', async () => {
  ...
  assert.throws(()=>A.actions.checkout({...A.cart,mode:'credit'}),/different store/);
  assert.equal(A.item(i.id),undefined); assert.equal(c.balance,0); assert.equal(i.stock,1);
  A.DB().settings.activeStore='st_main';
  assert.throws(()=>A.actions.checkout(cart(i,{lines:[...cart(i).lines,...cart(i).lines]})),/Insufficient/);
  assert.throws(()=>A.actions.checkout(cart(i,{lines:[{itemId:i.id,qty:1,price:101}]})),/new price/);
  i.deleted=true; assert.throws(()=>A.actions.checkout(cart(i)),/removed/);
...
test('B05–B07: displayed redemption equals persisted total; customer changes clear it; void returns points', async () => {
  ...
  const b=A.actions.checkout(A.cart); assert.equal(b.total,shown.total); assert.equal(c.points,0);
  A.DB().settings.loyaltyValue=2;
  A.actions.voidBill(b.id); assert.equal(c.points,20);
  A.cart.customerId=''; assert.equal(A.posTotals().redeem,0); assert.equal(A.cart.redeem,0);
  assert.throws(()=>A.actions.checkout(cart(i,{redeem:20})),/customer/);
});
...
test('B08: purchase cash, UPI, partial payable, and later payments reconcile once', async () => {
  ...
  A.actions.recordPurchase(s.id,[{itemId:i.id,qty:2,cost:60}],100,'','cash');
  assert.equal(A.stats.cashExpected().out,100);assert.equal(s.balance,20);
  A.actions.recordPurchase(s.id,[{itemId:i.id,qty:1,cost:60}],60,'','upi');
  assert.equal(A.stats.cashExpected().out,100);
  A.actions.paySupplier(s.id,20,'cash');assert.equal(A.stats.cashExpected().out,120);assert.equal(s.balance,0);
});
...
test('B09, B22: reject excess/nonfinite/negative payments and costs without partial mutations', async () => {
  ...
  A.save();const before=JSON.stringify(A.DB());
  assert.throws(()=>A.actions.takePayment(c.id,150,'cash'),/exceeds/);
  assert.throws(()=>A.actions.paySupplier(s.id,150,'cash'),/exceeds/);
  for(const cost of [-50,NaN,Infinity]) assert.throws(()=>A.actions.recordPurchase(s.id,[{itemId:i.id,qty:1,cost}],0));
  assert.throws(()=>A.actions.recordPurchase(s.id,[{itemId:i.id,qty:1,cost:60}],61),/exceeds/);
  assert.equal(JSON.stringify(A.DB()),before);
  A.actions.takePayment(c.id,100,'cash'); assert.equal(c.balance,0);
});
...
test('B10–B11: mixed GST, legacy bills, discounted paise and profit excluding tax', async () => {
  ...
  const bill=A.actions.checkout({lines:cart(a).lines.concat(cart(b).lines),mode:'cash'});
  ...
  const discounted=A.actions.checkout({lines:cart(a).lines.concat(cart(b).lines),mode:'cash',discount:0.01});
...
test('B30: a repaid credit sale becomes visible customer credit when voided', async () => {
  ...
  const b=A.actions.checkout(cart(i,{customerId:c.id,mode:'credit'}));
  A.actions.takePayment(c.id,100,'cash');A.actions.voidBill(b.id);
  ...
  A.actions.checkout(cart(i,{customerId:c.id,mode:'credit'}));assert.equal(c.balance,0);
});
...
test('B25: cashier action and editor boundaries', async () => {
  ...
  assert.throws(()=>A.actions.recordPurchase(s.id,[{itemId:i.id,qty:1,cost:1}],0),/Owner/);
  assert.throws(()=>A.actions.voidBill('any'),/Owner/);
  assert.throws(()=>A.importItemRows([{name:'new',price:1}]),/Owner/);
  assert.throws(()=>A.views.settings({}),/Owner/);
  A.actions.restock(i.id,1);A.actions.checkout(cart(i));assert.equal(i.stock,10);
});

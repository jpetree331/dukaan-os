...
test('VERIFY-08: modern purchase requires its ledger sources and a genuine cancellation record',async()=>{
 const {A}=await create(),i=item(A,{stock:0}),s=supplier(A);await A.save();const po=await A.actions.recordPurchase(s.id,[{itemId:i.id,qty:2,cost:60}],20,'');
 let bad=JSON.parse(JSON.stringify(A.DB()));bad.suppliers[0].ledger=bad.suppliers[0].ledger.filter(e=>e.kind!=='purchase');bad.suppliers[0].balance=-20;assert.throws(()=>A.validateData(bad),/purchase movement missing/);
 bad=JSON.parse(JSON.stringify(A.DB()));bad.purchases[0].cancelled=true;bad.purchases[0].cancellationId='invented';assert.throws(()=>A.validateData(bad),/cancellation record/);
 bad=JSON.parse(JSON.stringify(A.DB()));bad.suppliers[0].ledger=bad.suppliers[0].ledger.filter(e=>e.kind!=='initial_payment');bad.suppliers[0].balance=120;assert.throws(()=>A.validateData(bad),/initial payment movement missing/);
 await A.actions.paySupplier(s.id,10,'upi',{purchaseId:po.id});bad=JSON.parse(JSON.stringify(A.DB()));bad.suppliers[0].ledger=bad.suppliers[0].ledger.filter(e=>e.kind!=='payment');bad.suppliers[0].balance=100;assert.throws(()=>A.validateData(bad),/payment movement missing/);
});
test('VERIFY-08: supplier credit consumed by later purchases caps refunds; failed receipt remains retryable',async()=>{
 const {A}=await create(),i=item(A,{stock:0}),s=supplier(A);await A.save();const po=await A.actions.recordPurchase(s.id,[{itemId:i.id,qty:2,cost:60}],120,'');const r=await A.actions.returnPurchase({purchaseId:po.id,lines:[{lineId:po.lines[0].lineId,qty:2}],reason:'Supplier return'});
 await A.actions.recordPurchase(s.id,[{itemId:i.id,qty:1,cost:80}],0,'');assert.equal(A.supplierRefundDue(r),40);await assert.rejects(()=>A.actions.settleSupplierCredit(r.id,41,'cash','Too much'),/exceeds/);
 const before=JSON.stringify(A.DB()),commit=A.storage.commit;A.storage.commit=async()=>{throw new Error('Refund disk failure');};await assert.rejects(()=>A.actions.settleSupplierCredit(r.id,40,'cash','Cash received','refund_retry'),/disk/);assert.equal(JSON.stringify(A.DB()),before);A.storage.commit=commit;
 await A.actions.settleSupplierCredit(r.id,40,'cash','Cash received','refund_retry');assert.equal(s.balance,0);assert.equal(A.stats.cashExpected().net,-80);
});
test('VERIFY-08: supplier commands enforce actor/store and reject changed retry contents',async()=>{
 const {A}=await create(),i=item(A,{stock:0}),s=supplier(A);await A.save();const po=await A.actions.recordPurchase(s.id,[{itemId:i.id,qty:2,cost:60}],0,'','cash',{operationId:'fixed_purchase'});await assert.rejects(()=>A.actions.recordPurchase(s.id,[{itemId:i.id,qty:3,cost:60}],0,'','cash',{operationId:'fixed_purchase'}),/different contents/);
 const request={purchaseId:po.id,lines:[{lineId:po.lines[0].lineId,qty:1}],reason:'Return unused stock'};A.DB().staff.push({id:'cashier_supplier',name:'Cashier',role:'cashier'});A.DB().session.staffId='cashier_supplier';await A.save();await assert.rejects(()=>A.actions.returnPurchase(request),/permission|owner|allowed|access/i);
 A.DB().session.staffId=A.DB().staff.find(s=>s.role==='owner').id;A.DB().stores.push({id:'other_supplier_store',name:'Other store'});A.DB().settings.activeStore='other_supplier_store';await A.save();await assert.rejects(()=>A.actions.returnPurchase(request),/not found/);
});

...
(function(w){
 ...
 A.cashDaySummary=function(dayTs=Date.now()){
  ...
  return {in:incoming,out:outgoing,net:A.round2(incoming-outgoing),billCash:sum(['sale','void']),payCash:sum(['collection']),refundCash:-sum(['customer_refund']),supplierRefundCash:sum(['supplier_refund']),supplierOut:-sum(['purchase','supplier_payment']),manualNet:sum(['expense','withdrawal','cash_correction']),businessDate:day};
 };
 ...
 A.validateCashData=function(db){
  ...
  for(const [name,rows] of [['baseline',db.cashBaseline],['movement',db.cashMovements]])for(const r of rows){
   const source=r&&sources.get(r.key);if(!source||seen.has(r.key)||['kind','sourceId','storeId','delta','sourceAt'].some(k=>r[k]!==source[k]))fail('cash source missing, repeated or changed');seen.add(r.key);money(r.delta,'cash movement',true);A.number(r.sourceAt,'source date',0,8640000000000000);
   if(name==='movement'){
  ...
  const corrections=new Map(),correctionIds=new Set();for(const c of db.cashCloseCorrections){const s=shifts.get(c.shiftId),previous=corrections.get(c.shiftId)||s?.close;if(!s?.close||!id(c.id)||correctionIds.has(c.id)||c.storeId!==s.storeId||c.corrects!==previous.id||typeof c.note!=='string'||c.note.trim().length<3)fail('invalid close correction');correctionIds.add(c.id);money(c.counted,'corrected count');A.number(c.at,'correction date',s.close.at,8640000000000000);if(c.expected!==s.close.expected||c.variance!==A.round2(c.counted-c.expected))fail('close correction totals differ');corrections.set(c.shiftId,c);}
 };
 ...
 A.cashOpenDialog=function(){
  A.requirePermission('settings');const store=A.DB().stores.find(s=>s.id===A.S()),last=A.cashSessions().at(-1),operationId=A.uid('cash_shift'),body=A.el('<div><p>Count the cash physically in the till. After the first opening, cash transactions require an open shift in every store. Earlier transactions stay in history and are not added to this opening float.</p>'+field('cashOpening','Opening cash',last?.close?A.reportedCashClose(last).counted:0,'number')+field('cashTimezone','Store timezone',store.timeZone || 'Asia/Kolkata')+field('cashOpeningNote','Opening count note')+'</div>');
  if(last)body.querySelector('#cashTimezone').readOnly=true;
  A.modal({title:'Open cash shift',body,buttons:[{label:'Cancel',cls:'ghost'},{label:'Open shift',cls:'pri',fn:async()=>{await A.actions.openCashShift({openingFloat:Number(A.$('#cashOpening',body).value),timeZone:A.$('#cashTimezone',body).value,note:A.$('#cashOpeningNote',body).value,operationId});A.render();}}]});
 };
 A.cashEntryDialog=function(kind='expense',corrects=''){
  A.requirePermission('settings');const original=corrects&&(A.DB().cashManual || []).find(r=>r.id===corrects),operationId=A.uid('cash_manual'),body=A.el('<div><p>'+(original?'Record the opposite cash movement now. The original entry and any closed shift stay unchanged.':'Record cash already taken out of the till.')+'</p>'+field('cashAmount','Cash amount',original?Math.abs(original.delta):'','number')+(original?'':'<div class="field"><label for="cashReason">Reason</label><select class="inp" id="cashReason">'+['supplies','transport','utilities','owner','other'].map(r=>'<option '+(r===(kind==='withdrawal'?'owner':'other')?'selected':'')+'>'+r+'</option>').join('')+'</select></div>')+field('cashEntryNote','Reason / receipt note')+'</div>');
  if(original)body.querySelector('#cashAmount').readOnly=true;
  A.modal({title:original?'Reverse cash entry':kind==='withdrawal'?'Owner withdrawal':'Cash expense',body,buttons:[{label:'Cancel',cls:'ghost'},{label:'Record cash movement',cls:'pri',fn:async()=>{await A.actions.cashEntry({kind:original?'cash_correction':kind,amount:Number(A.$('#cashAmount',body).value),reason:original?'reversal':A.$('#cashReason',body).value,note:A.$('#cashEntryNote',body).value,corrects,operationId});A.render();}}]});
 };
 A.cashCloseDialog=function(shiftId,correction=false){
  A.requirePermission('settings');const shift=A.cashSessions().find(s=>s.id===shiftId),summary=A.cashShiftSummary(shift),operationId=A.uid(correction?'cash_count_correction':'cash_close'),body=A.el('<div><p>Expected cash: <b>'+esc(money(summary.expected))+'</b> · '+esc(shift.timeZone)+'</p><p>'+(correction?'This appends a correction to the recorded count. It does not move cash or change the original signed-off count.':'Closing fixes this shift’s movement list and expected amount. Later refunds and corrections belong to a later open shift.')+'</p>'+field('cashCounted','Physically counted cash',correction?A.reportedCashClose(shift).counted:'','number')+field('cashClosingNote',correction?'Correction reason':'Closing note')+'</div>');
  A.modal({title:correction?'Correct recorded closing count':'Close cash shift',body,buttons:[{label:'Cancel',cls:'ghost'},{label:correction?'Save count correction':'Close shift',cls:'pri',fn:async()=>{const request={shiftId,counted:Number(A.$('#cashCounted',body).value),note:A.$('#cashClosingNote',body).value,operationId};if(correction)await A.actions.correctCashClose(request);else await A.actions.closeCashShift(request);A.render();}}]});
 };
 A.cashShiftDetail=function(id){
  A.requirePermission('settings');const shift=A.cashSessions().find(s=>s.id===id),summary=A.cashShiftSummary(shift),reported=shift.close&&A.reportedCashClose(shift),manual=A.DB().cashManual || [],body=A.el('<div><p>Opened '+esc(A.cashTime(shift.openedAt,shift.timeZone))+' · '+esc(shift.timeZone)+' · float '+esc(money(shift.openingFloat))+'</p><p><b>Expected '+esc(money(summary.expected))+'</b>'+(shift.close?' · closed '+esc(A.cashTime(shift.close.at,shift.timeZone))+' · original count '+esc(money(shift.close.counted))+' · reported count '+esc(money(reported.counted))+' · variance '+esc(money(reported.variance)):' · Open')+'</p><p>Latest 50 movements; CSV includes every movement.</p><div class="tbl-wrap"><table class="tbl"><thead><tr><th>Posted</th><th>Movement</th><th>Cash change</th><th></th></tr></thead><tbody>'+summary.movements.slice(-50).map(m=>{const entry=manual.find(r=>r.id===m.sourceId&&r.kind===m.kind);return '<tr><td>'+esc(A.cashTime(m.at,shift.timeZone))+'</td><td>'+esc(m.note || m.kind)+'</td><td>'+esc(money(m.delta))+'</td><td>'+(entry&&!manual.some(r=>r.corrects===entry.id)?'<button class="btn xs" data-reverse-cash="'+esc(entry.id)+'">Reverse cash entry</button>':'')+'</td></tr>';}).join('')+'</tbody></table></div>'+((A.DB().cashCloseCorrections || []).filter(c=>c.shiftId===id).map(c=>'<p>Count correction '+esc(A.cashTime(c.at,shift.timeZone))+': '+esc(money(c.counted))+' · '+esc(c.note)+'</p>').join(''))+'</div>');
  const modal=A.modal({title:'Cash shift details',body,wide:true,buttons:[{label:'Movements CSV',keepOpen:true,fn:()=>A.download(A.toCSV([['Posting UTC','Business date','Timezone','Source','Cash change','Actor','Note'],...summary.movements.map(m=>[new Date(m.at).toISOString(),m.businessDate,shift.timeZone,m.key,m.delta,m.staffId,m.note])]),'cash-shift-'+id+'.csv','text/csv')},shift.close?{label:'Correct recorded count',fn:()=>A.cashCloseDialog(id,true)}:{label:'Close shift',fn:()=>A.cashCloseDialog(id)},{label:'Close',cls:'ghost'}]});
  body.addEventListener('click',e=>{const b=e.target.closest('[data-reverse-cash]');if(!b)return;if(!A.openCashSession()){A.toast('warn','Open a later cash shift before reversing a movement');return;}modal.close();A.cashEntryDialog('cash_correction',b.dataset.reverseCash);});
 };
 A.cashDashboard=function(){
  A.requirePermission('settings');const shifts=A.cashSessions(),open=A.openCashSession(),body=A.el('<div>'+(open?'<p><b>Open shift · expected cash '+esc(money(A.cashShiftSummary(open).expected))+'</b></p>':'<p>No cash shift is open in this store.</p>')+'<p>Closed shifts retain their original cash movements. Late entries post to the currently open shift; closing-count corrections are separate records.</p>'+shifts.slice().reverse().map(s=>{const r=s.close&&A.reportedCashClose(s);return '<div class="list-row"><span style="flex:1">'+esc(A.cashTime(s.openedAt,s.timeZone))+' · '+(s.close?'Closed':'Open')+'<br><small>Float '+esc(money(s.openingFloat))+' · expected '+esc(money(A.cashShiftSummary(s).expected))+(r?' · count '+esc(money(r.counted))+' · variance '+esc(money(r.variance)):'')+'</small></span><button class="btn sm" data-cash-shift="'+esc(s.id)+'">Details</button></div>';}).join('')+'</div>');
  const modal=A.modal({title:'Cash shifts',body,wide:true,buttons:[open?{label:'Cash expense',fn:()=>A.cashEntryDialog('expense')}:null,open?{label:'Owner withdrawal',fn:()=>A.cashEntryDialog('withdrawal')}:null,open?{label:'Close shift',cls:'pri',fn:()=>A.cashCloseDialog(open.id)}:{label:'Open cash shift',cls:'pri',fn:()=>A.cashOpenDialog()},{label:'Closing summary CSV',keepOpen:true,fn:()=>A.download(A.toCSV([['Shift','Timezone','Opened UTC','Closed UTC','Opening float','Expected','Original count','Reported count','Reported variance'],...shifts.map(s=>{const r=s.close&&A.reportedCashClose(s);return [s.id,s.timeZone,new Date(s.openedAt).toISOString(),s.close?new Date(s.close.at).toISOString():'',s.openingFloat,A.cashShiftSummary(s).expected,s.close?.counted ?? '',r?.counted ?? '',r?.variance ?? ''];})]),'cash-shift-closings.csv','text/csv')},{label:'Close',cls:'ghost'}]});body.addEventListener('click',e=>{const b=e.target.closest('[data-cash-shift]');if(b){modal.close();A.cashShiftDetail(b.dataset.cashShift);}});
 };
})(window);

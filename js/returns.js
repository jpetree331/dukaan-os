...
(function(w){
 ...
 A.showReturn=function(ret){
  ...
  A.modal({title:window.App.moneyLiteral('Return for bill #')+ret.billNo,body,buttons:[{label:window.App.moneyLiteral('Download return note'),cls:'ghost',keepOpen:true,fn:()=>A.download(A.returnDocument(ret),'return-'+ret.id+'.txt','text/plain')},A.returnRefundDue(ret)>0?{label:window.App.moneyLiteral('Record refund paid'),cls:'pri',fn:()=>A.refundDialog(ret.id)}:null,{label:window.App.moneyLiteral('Close'),cls:'ghost'}]});
 };
